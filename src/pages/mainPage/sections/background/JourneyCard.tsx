    ...
    return (
        ...
        >
            <div className={`flex flex-col items-center text-center mx-auto  w-full sm:w-auto ${alignRight? "md:text-left md:items-start":"md:text-right md:items-end"}`}>
                ...
                <span className="text-cyan-600 font-medieval font-semibold text-[clamp(0.95rem,1.2vw,1.15rem)] tracking-[0.15em] uppercase">
                    {item.year}
                </span>

                <div className="w-16 h-[1px] bg-cyan-300 mt-1 md:opacity-50 opacity-0"></div>
                {item.type === "work" ? (
                    <Briefcase className="w-5 h-5 mt-1 text-cyan-600 opacity-80 " />
                ) : (
                    <GraduationCap className="w-5 h-5 mt-1 text-cyan-600 opacity-80" />
                )}
            </div>
